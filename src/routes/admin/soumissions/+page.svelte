	...
	<div class="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
		{#if loading}
			<div class="p-5 space-y-3">
				{#each Array(6) as _}
					<div class="h-14 bg-zinc-800 rounded animate-pulse"></div>
				{/each}
			</div>
		...
		{:else}
						...
						{#each filtered as s}
							<tr class="hover:bg-zinc-800/40 transition cursor-pointer">
								<td class="px-5 py-3 text-zinc-400 font-mono text-xs">
									<a href={`/admin/soumissions/${s.id}`} class="block">
								...
								<td class="px-5 py-3 text-zinc-300">
									<a href={`/admin/soumissions/${s.id}`} class="block">{s.projet_type || '—'}</a>
								</td>
								<td class="px-5 py-3 text-right text-white font-semibold">
									<a href={`/admin/soumissions/${s.id}`} class="block">{fmtMoney(s.total || 0)}</a>
								</td>
								<td class="px-5 py-3">
									<a href={`/admin/soumissions/${s.id}`} class="block">
										<span
								...
								<td class="px-5 py-3 text-zinc-500 text-xs">
									<a href={`/admin/soumissions/${s.id}`} class="block">{fmtDate(s.created_at)}</a>
								</td>
								<td class="px-5 py-3">
									<a
			...
			<ul class="md:hidden divide-y divide-zinc-800">
				{#each filtered as s}
					<li>
						<a href={`/admin/soumissions/${s.id}`} class="flex items-start gap-3 p-4 hover:bg-zinc-800/40 transition">
