<script lang="ts">
	...
	$: hasPriced = quote.amount_cents > 0 && !!latestOffer;
...
<div class="min-h-screen bg-zinc-950 text-white px-4 py-10">
	<div class="max-w-3xl mx-auto">
		...
		<div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-10">
			...
			{#if !hasPriced}
				<!-- Submission stage: business hasn't priced yet -->
			...
			{:else if canSign}
				<div class="grid sm:grid-cols-2 gap-6">
					...
					<form
						...
						action="?/sign"
						use:enhance
						class="bg-zinc-950 border border-zinc-800 rounded-lg p-5"
					>
					...
					{#if canCounter}
						...
						>
							...
							<input
								type="hidden"
								name="amount_cents"
								value={Math.round(Number.parseFloat(counterAmount || '0') * 100)}
							/>
