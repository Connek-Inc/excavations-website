<script lang="ts">
	...
	import { SITE, breadcrumbJsonLd, localBusinessJsonLd } from '$lib/seo/seo-config';
	...
	$: data =
		lang === 'fr'
			? {
					...
					pagetitle:
						"L'Importance du Drain Français au Québec | Guide Complet 2026 | Mini Excavations Érable",
					description:
						"Guide complet 2026 sur l'importance du drain français pour votre maison au Québec. Causes d'infiltration, signes d'alerte, prix, processus d'installation. Soumission gratuite.",
					keywords:
						"drain français Québec, importance drain français, installation drain français Montréal, infiltration eau sous-sol Québec, gel-dégel fondation",
...
<p class='mb-4'>Plus de 15 ans d'expertise au Québec, certifications RBQ, NEQ, CCQ et CMMTQ, garantie écrite, soumissions gratuites en 24h. En possédant nos propres machines, nous offrons des prix compétitifs sans compromis sur la qualité.</p>`
			...
			: lang === 'es'
				? {
						...
						pagetitle:
							'Importancia del Drenaje Francés en Quebec | Guía 2026 | Mini Excavations Érable',
						description:
							'Guía completa 2026 sobre la importancia del drenaje francés en Quebec. Causas de infiltración, señales, precios, proceso de instalación.',
						keywords:
							'drenaje francés Quebec, instalación drenaje Montreal, infiltración agua sótano, hielo-deshielo cimientos',
...
<p class='mb-4'>+15 años de experiencia, certificaciones RBQ, NEQ, CCQ, CMMTQ, garantía escrita, cotización gratuita en 24h.</p>`
				...
				: {
						...
						pagetitle:
							'French Drain Importance in Quebec | 2026 Complete Guide | Mini Excavations Érable',
						description:
							'Complete 2026 guide on French drain importance for Quebec homes. Infiltration causes, warning signs, pricing, installation process. Free quote.',
						keywords:
							'french drain Quebec, french drain importance, installation Montreal, basement water infiltration, freeze-thaw foundation',
...
<p class='mb-4'>15+ years experience, RBQ, NEQ, CCQ, CMMTQ certified, written warranty, free 24h quote.</p>`
	...
	$: articleJsonLd = {
		...
		'@type': 'Article',
		mainEntityOfPage: { '@type': 'WebPage', '@id': `${SITE.url}/blog/importance` },
		headline: data.title,
	...
	$: jsonLd = [
		...
		breadcrumbJsonLd([
			...
			{ name: data.title, url: `${SITE.url}/blog/importance` }
		])
	];
</script>
...
<main class="bg-white dark:bg-black text-black dark:text-white min-h-screen">
	<article itemscope itemtype="https://schema.org/Article" class="pt-32 pb-16">
		<header class="text-center px-4 mb-12 max-w-4xl mx-auto">
			<p class="text-sm md:text-base text-[#febd17] font-bold tracking-wider uppercase mb-4">
				<time datetime="2024-07-04" itemprop="datePublished">{data.subtitle}</time>
				<span class="text-gray-400 mx-2">/</span>{data.subtitle2}
			</p>
			<h1 class="font-black text-3xl md:text-5xl lg:text-6xl tracking-tight" itemprop="headline">
				{data.title}
			</h1>
		</header>

		<figure class="container max-w-6xl mx-auto px-4 mb-12">
			<img
				src={importance}
				alt={lang === 'fr'
					? "Installation de drain français au Québec par Mini Excavations Érable"
					: lang === 'es'
						? 'Instalación de drenaje francés en Quebec por Mini Excavations Érable'
						: 'French drain installation in Quebec by Mini Excavations Érable'}
				width="1200"
				height="675"
		...
		<div
			class="container max-w-4xl mx-auto px-6 prose prose-lg dark:prose-invert"
			itemprop="articleBody"
		>
			{@html data.content}
		</div>

		<footer class="container max-w-4xl mx-auto px-6 mt-16 pt-8 border-t border-gray-200 dark:border-zinc-800">
			<div class="flex items-center gap-4" itemprop="author" itemscope itemtype="https://schema.org/Organization">
				<img src={logo} alt="Mini Excavations Érable" class="w-12 h-12 rounded-full" />
				<div>
					<p class="font-bold text-lg" itemprop="name">Mini Excavations Érable</p>
					<p class="text-gray-500 dark:text-gray-400 text-sm">{data.author_desc}</p>
				</div>
			</div>
		</footer>
	</article>
