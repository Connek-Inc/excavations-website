<script lang="ts">
	...
	import { ArrowLeft, ArrowRight, Lock, Mail, Loader2, User as UserIcon, Phone } from 'lucide-svelte';
...
<svelte:head>
	<title>Créer un compte — Mini Excavations Érable</title>
	...
	<main class="flex-1 flex items-center justify-center px-4 py-12">
		<div class="w-full max-w-md">
			<div class="text-center mb-8">
				...
				<h1 class="text-3xl font-black mb-2">Créer un compte</h1>
				<p class="text-sm text-zinc-500">Suivez vos soumissions et historique de projets</p>
			</div>
			...
			>
				...
				<div>
					<label for="name" class="block text-xs font-bold uppercase tracking-wider text-zinc-400 mb-2">Nom complet</label>
					<input
						id="name"
						type="text"
						name="name"
				...
				<div>
					<label for="tel" class="block text-xs font-bold uppercase tracking-wider text-zinc-400 mb-2">Téléphone (optionnel)</label>
					<div class="relative">
				...
				<div>
					<label for="pwd" class="block text-xs font-bold uppercase tracking-wider text-zinc-400 mb-2">Mot de passe (min. 8 car.)</label>
					<div class="relative">
				...
				>
					{#if loading}
						<Loader2 class="h-4 w-4 animate-spin" /> Création…
					{:else}
						Créer mon compte <ArrowRight class="h-4 w-4" />
					{/if}
				</button>

				<p class="text-xs text-zinc-500 text-center">
					Déjà un compte? <a href="/compte/login" class="text-amber-400 hover:text-amber-300">Se connecter</a>
				</p>
			</form>
		</div>
	</main>
