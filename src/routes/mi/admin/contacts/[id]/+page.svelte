...
<div class="space-y-6 max-w-5xl">
	<a href="/mi/admin/contacts" class="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-zinc-400 hover:text-[#febd17] transition-colors">
		<ArrowLeft class="w-4 h-4" /> Retour à la liste
	</a>

	<div class="flex items-start justify-between flex-wrap gap-4">
		<div class="flex items-center gap-4">
			<div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#febd17] to-yellow-600 flex items-center justify-center text-black font-black text-2xl shadow-lg shadow-yellow-500/30">
		...
		<form method="POST" action="?/delete" use:enhance>
			<button
				type="submit"
				on:click={(e) => !confirm('Supprimer définitivement ce contact ?') && e.preventDefault()}
				class="px-4 py-2 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 font-medium text-sm hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors flex items-center gap-2"
			>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		...
		<div class="lg:col-span-2 space-y-6">
			<div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6">
				...
				{#if contact.subject}
					<p class="font-semibold mb-2">{contact.subject}</p>
				{/if}
			...
			<form
				...
				class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6 space-y-4"
			>
				<h2 class="font-bold text-lg">Gestion</h2>

				<div class="space-y-2">
					<Label forId="status">Statut</Label>
					<select
						id="status"
						name="status"
						bind:value={status}
						class="w-full h-11 px-4 rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-sm focus:outline-none focus:ring-2 focus:ring-[#febd17]"
					>
				...
				<div class="space-y-2">
					<Label forId="notes">Notes internes</Label>
					<Textarea
						id="notes"
						name="notes"
						bind:value={notes}
				...
				<button
					type="submit"
					disabled={saving}
					class="px-6 py-2.5 rounded-xl bg-[#febd17] text-black font-bold text-sm hover:bg-yellow-400 transition-colors flex items-center gap-2 disabled:opacity-60"
				>
		...
		<div class="space-y-4">
			...
			<div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6 space-y-2 text-xs text-gray-500 dark:text-zinc-400">
				...
				<div class="flex justify-between">
					<span>Source</span>
					<span class="font-medium text-gray-700 dark:text-zinc-300">{contact.source ?? 'website'}</span>
				</div>
				<div class="flex justify-between">
					<span>Langue</span>
					<span class="font-medium text-gray-700 dark:text-zinc-300 uppercase">{contact.language ?? 'fr'}</span>
				</div>
				{#if contact.ipAddress}
					<div class="flex justify-between">
						<span>IP</span>
						<span class="font-mono text-gray-700 dark:text-zinc-300 text-[10px]">{contact.ipAddress}</span>
					</div>
				{/if}
			</div>
		</div>
