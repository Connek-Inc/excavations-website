<script lang="ts">
	...
	const statusMap: Record<string, { label: string; color: string }> = {
		...
		quoted: { label: 'Devis', color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-400' },
...
<svelte:head><title>Contacts — Admin</title></svelte:head>
...
<div class="space-y-6">
	<div>
		...
		<p class="text-gray-500 dark:text-zinc-400 mt-1">{data.total} contact{data.total !== 1 ? 's' : ''}</p>
	</div>
	...
	<div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-4 flex flex-col md:flex-row gap-3">
		...
		<select bind:value={statusValue} on:change={applyFilters} class="h-11 px-4 rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-sm focus:outline-none focus:ring-2 focus:ring-[#febd17]">
		...
		<button on:click={applyFilters} class="px-6 py-2 rounded-xl bg-[#febd17] text-black font-bold text-sm hover:bg-yellow-400 flex items-center justify-center gap-2">
		...
		{:else}
							...
							<tr class="border-b border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-950">
								<td class="p-4">
									<div class="flex items-center gap-3">
										<div class="w-10 h-10 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-900 flex items-center justify-center text-white font-bold text-xs">
										...
										<div class="min-w-0">
											...
											{#if c.phone}<p class="text-xs text-gray-500 dark:text-zinc-400 truncate flex items-center gap-1">
								...
								<td class="p-4">
									<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold {info.color}">{info.label}</span>
								</td>
								<td class="p-4 hidden lg:table-cell text-gray-500 dark:text-zinc-400 text-xs">{formatDate(c.createdAt)}</td>
								<td class="p-4 text-right">
									<div class="flex justify-end gap-1">
										<a href={`/mi/admin/contacts/${c.id}`} class="p-2 rounded-lg hover:bg-yellow-100 dark:hover:bg-yellow-500/10 text-gray-600 dark:text-zinc-400 hover:text-[#febd17]" aria-label="Voir">
											<Eye class="w-4 h-4" />
										</a>
										<form method="POST" action="?/delete" use:enhance>
											<input type="hidden" name="id" value={c.id} />
											<button type="submit" class="p-2 rounded-lg hover:bg-red-100 dark:hover:bg-red-500/10 text-gray-600 dark:text-zinc-400 hover:text-red-500" on:click={(e) => !confirm('Supprimer ce contact ?') && e.preventDefault()} aria-label="Supprimer">
			...
			{#if totalPages > 1}
				<div class="flex justify-between items-center p-4 border-t border-gray-200 dark:border-zinc-800">
					<p class="text-sm text-gray-500 dark:text-zinc-400">Page {data.page} sur {totalPages}</p>
					<div class="flex gap-2">
						<button on:click={() => gotoPage(data.page - 1)} disabled={data.page <= 1} class="px-4 py-2 rounded-lg border border-gray-200 dark:border-zinc-800 text-sm font-medium disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-zinc-800">
							Précédent
						</button>
						<button on:click={() => gotoPage(data.page + 1)} disabled={data.page >= totalPages} class="px-4 py-2 rounded-lg border border-gray-200 dark:border-zinc-800 text-sm font-medium disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-zinc-800">
							Suivant
						</button>
					</div>
				</div>
