// ╔════════════════════════════════════════════════════════════════════════════
// ║ Submission/lead email templates for excavationserable.com (Mini Excavations
// ║ Érable). Rendered server-side by server.js and sent via Resend so a lead is
// ║ never lost even if the Connek API / workers are down. Same templates used by
// ║ the SvelteKit site's fallback path.
// ╚════════════════════════════════════════════════════════════════════════════
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06);">
        ...
        <tr><td style="padding:18px 28px;background:#f9fafb;border-top:1px solid #e5e7eb;color:#6b7280;font-size:12px;">
          Mini Excavations Érable · (514) 830-9973 · Envoyé via Connek
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function clientHtml(d) {
	...
	const inner = `
		...
		<p style="margin:0 0 16px;font-size:15px;line-height:1.5;">Bonjour ${esc(name)}, merci pour votre demande${d.reference ? ` (réf. <b>${esc(d.reference)}</b>)` : ''}. Notre équipe la traite et vous reviendra rapidement.</p>
		<table role="presentation" cellpadding="0" cellspacing="0">
			${row('Catégorie', d.category || d.projet_type)}
			${row('Description', d.project_description || d.projet_description)}
		</table>
		${btn(tracking, 'Suivre ma demande')}
		<p style="margin:16px 0 0;color:#6b7280;font-size:13px;">Une question? Appelez-nous au (514) 830-9973.</p>`;
	return {
		subject: d.subject || `Nous avons reçu votre demande${d.reference ? ` — ${d.reference}` : ''} — Mini Excavations Érable`,
...
function businessHtml(d) {
	const tracking = d.tracking_url || (d.token ? `https://excavationserable.com/soumission/${d.token}` : '');
	const inner = `
		<h1 style="margin:0 0 8px;font-size:22px;">Nouvelle demande de soumission</h1>
		<p style="margin:0 0 16px;font-size:15px;line-height:1.5;">${d.reference ? `Référence <b>${esc(d.reference)}</b>. ` : ''}Un client vient de soumettre une demande sur excavationserable.com.</p>
		<table role="presentation" cellpadding="0" cellspacing="0">
			...
			${row('Zone / Ville', d.project_address || d.projet_adresse)}
			${row('Catégorie', d.category || d.projet_type)}
			${row('Description', d.project_description || d.projet_description)}
		</table>
		${btn(tracking, 'Ouvrir la soumission')}`;
	return {
		subject: d.subject || `Nouvelle demande${d.reference ? ` — ${d.reference}` : ''}: ${d.client_first_name || d.client_name || ''}`.trim(),
		html: shell('Nouvelle demande', inner)
	};
}

/**
 * Render a submission email.
 * @param {string} kind  'submission_client' | 'submission_business'
 * @param {object} data  structured submission fields
